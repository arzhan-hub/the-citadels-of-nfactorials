...
2. **Цитадель:** отсылка к *Citadel of Ricks* — гигантский межпространственный мегаполис, созданный Риками из разных вселенных. Изначально Цитадель управлялась Советом Риков — элитой «самых умных Риков». Это место, где у Риков был доступ к информации.
...
* **Символ (!):** В геометрию правой части буквы вписан **силуэт восклицательного знака**. Вместе с буквой N это образует математический символ Факториала ($n!$) — знак стремительного, взрывного роста.
* **Вектор вверх:** Центральная часть логотипа образует стрелку, устремленную ввысь, что означает постоянный прогресс.
* **Палитра:**
...
3. **Серверные API**: добавил `/api/*` роуты, чтобы все внешние сервисы вызывались с сервера.
...
**Альтернатива**: использование gpt mini 5, но ответы дольше

### Локализация
**Решение**: ручной словарь имён на основе озвучки Сыендука  
**Альтернатива**: авто‑перевод через AI  
**Почему**: сохранение локализованных имён, которые были закреплены в русской озвучке  
**Trade‑off**: неполное покрытие

### Portal Cursor
**Решение**: эффект активен только на ссылках в шапке и отключается вручную  
**Альтернатива**: глобальный эффект на всех кликах (слишком назойливо) 
**Почему**: стабильная производительность и меньше отвлечения  
**Trade‑off**: меньше “магии” за пределами навигации и переход только в другую категорию
