...
export default function Home() {
  ...
  useEffect(() => {
    const fetchCharacters = async () => {
      setLoading(true);
      try {
        const res = await fetch(`/api/characters?name=${search}`);
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
            {characters.length > 0 ? (
              characters.map((char) => (
                <div 
                  key={char.id} 
                  className="bg-gray-800 rounded-2xl overflow-hidden hover:shadow-2xl hover:shadow-green-500/20 transition-all duration-300 hover:-translate-y-2 border border-gray-700"
                >
                  ...
                  <div className="p-6">
                    ...
                    <div className="space-y-2">
                      <div className="text-sm text-gray-400">
                        <span className="block text-xs uppercase text-gray-500 font-semibold">Last Location</span>
                        {char.location.name}
                      </div>
                    </div>
                  </div>
